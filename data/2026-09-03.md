...
TL;DR: The paper investigates vulnerabilities in agentic AI systems regarding hidden-context leakage during sensitive data processing and develops the concept of context-inference attacks, demonstrating significant leakage under various conditions.


<details>
  <summary>Details</summary>
Motivation: To address the overlooked privacy risks associated with agentic AI systems processing sensitive data, particularly regarding hidden contexts formed by tool calls.

Method: The authors introduce and formalize context-inference attacks through a security game, evaluating these attacks in varying settings with different levels of attacker knowledge and context retrieval methods.

Result: The study shows that hidden-context leakage is exploitable in agentic systems, achieving high attack success rates under several scenarios, including known and unknown contexts.

Conclusion: Even with controls in place, agentic AI systems remain vulnerable to context-inference attacks, posing significant privacy risks that warrant further investigation.
...
TL;DR: The paper presents the Private Computation Space (PCS), an open-source ML system designed for agriculture that utilizes Federated Learning and maintains data privacy, addressing farmers' concerns while enhancing model performance in real-world applications.


<details>
  <summary>Details</summary>
Motivation: To overcome farmers' privacy concerns about data sharing, which hinder the adoption of AI in agriculture.

Method: The PCS system employs multi-cluster orchestration, asynchronous Federated Learning, Differential Privacy, and Trusted Execution Environments, ensuring data privacy and utility on rural infrastructures.

Result: PCS was evaluated with workloads for monitoring nitrogen and predicting evapotranspiration, achieving high accuracy metrics (DSC of 0.71 and $R^2$ of 0.84), outperforming traditional single-site models by significant margins while safeguarding privacy.

Conclusion: The PCS demonstrates that a privacy-preserving ML framework can effectively support agricultural practices, addressing both the technical and practical concerns of farmers.
...
TL;DR: This paper critiques the reliance on triple-F1 scores for evaluating knowledge-graph extraction tools, revealing significant discrepancies in matching protocols and proposing CTIForge to isolate matcher effects, ultimately improving precision for selected configurations.


<details>
  <summary>Details</summary>
Motivation: The study aims to assess and improve the evaluation methods for knowledge-graph extraction systems by revealing inconsistencies in current scoring practices and improving precision through specific configurations.

Method: The authors analyzed twelve systems and re-scored outputs based on varying matching protocols, developed the CTIForge framework to isolate extraction effects, and tested various configurations to assess validation impacts on performance metrics.

Result: The study found that previous scoring methods often misrepresent system performance, with a notable divergence in F1 scores. CTIForge improved precision for all four tested hosted backbones and highlighted the effect of validation protocols on extraction performance.

Conclusion: The results suggest that significant improvements in extraction accuracy can be achieved by re-evaluating scoring methods and employing new validation techniques, with implications for future knowledge-graph extraction tool assessments.
...
TL;DR: AI coding agents are evolving into collaborative teammates, but existing protocols leak sensitive skill information. This paper introduces Skill-as-API, a protocol for enhanced privacy in AI coordination by limiting public skill details and ensuring that sensitive prompts remain local.


<details>
  <summary>Details</summary>
Motivation: To address the privacy risks associated with AI coding agents sharing sensitive skill information during collaboration.

Method: Introduction of the Skill-as-API protocol, which restricts the accessible information about a skill to only its name, description, input/output schemas, and trust tier, while keeping the skill body local to the owner.

Result: A reduced risk of intellectual property leakage and improved privacy in coordination between AI agents, achieved through an open-source Python implementation with efficient reconnection latencies.

Conclusion: The Skill-as-API protocol enhances privacy in AI collaboration by structurally limiting exposure of sensitive skills and prompting, successfully demonstrated through a case study of agents reviewing a pull request.
...
TL;DR: This paper investigates the interaction between Model Context Protocol (MCP) and Agent2Agent (A2A) delegation regarding safety properties in a controlled test setting. It assesses the behavior of agents using both configurations and examines how labeling affects message sharing outcomes.


<details>
  <summary>Details</summary>
Motivation: To understand the behavior of safety properties when both MCP and A2A are utilized by a single agent, and to explore the impact of labeling scenarios on data sharing outcomes.

Method: The study employs a controlled testbed with a structured three-arm design to evaluate the effect of three different headers (CONFIDENTIAL, unlabeled, and PUBLIC - OK TO SHARE) across 10 record scenarios, using a deterministic scoring approach and a set of performance metrics (mean, median, sign counts).

Result: The analysis resulted in high variance in outcomes depending on model selection and header type, with some models showing stronger associations between PUBLIC labeling and increased message sharing compared to unlabeled scenarios, but findings were limited by a lack of protective effect for CONFIDENTIAL labels.

Conclusion: While associations were noted between labeling and agent behavior, the results do not establish causality or general effectiveness across different models; the findings suggest further investigation is needed to draw robust conclusions.
...
TL;DR: The paper discusses a new black-box membership inference attack (MIA) framework for TTS models, addressing unique challenges in query generation and representation engineering. It reveals significant privacy risks related to voice synthesis.


<details>
  <summary>Details</summary>
Motivation: To evaluate and mitigate privacy risks in TTS models fine-tuned on private datasets, as existing MIAs are inadequate for the specific challenges posed by TTS.

Method: An MIA framework was developed, focusing on query generation and representation engineering. Key criteria were established for evaluating query effectiveness, leading to the identification of 'recitation' as the most effective query type. Multi-level speech representations were obtained for thorough comparisons.

Result: The framework revealed high privacy leakage indicators: speaker-level AUC exceeded 0.80 and approached 1.0, while record-level AUC varied from 0.80 to 0.90 across different settings, even under challenging conditions.

Conclusion: The study highlights significant vulnerabilities in TTS models regarding privacy, emphasizing the need to address specific speech characteristics that increase risks of membership inference.
...
TL;DR: HEAT, a training method for fully homomorphic encryption, allows fine-tuning of iteration counts, significantly reducing latency and improving performance for encrypted language models.


<details>
  <summary>Details</summary>
Motivation: Current FHE methods for running language models are slow due to fixed iteration counts leading to excessive bootstrapping, which hampers performance.

Method: HEAT introduces learnable iteration counts per nonlinearity, allowing adaptation to task objectives and approximation errors during training without major architectural changes.

Result: In encrypted GPT-2 decoding, HEAT achieves a 3.1x reduction in iterations, 1.6x fewer bootstraps, and 1.4x lower end-to-end latency, while enhancing decode agreement.

Conclusion: HEAT optimizes FHE performance for language models, enabling efficient inference and significant reductions in latency.
...
TL;DR: This paper explores using Behavior Trees (BTs) instead of Abstract Syntax Trees (ASTs) for representing Java code in LLM-based vulnerability detection, finding BTs offer advantages in recall and performance within context limits.


<details>
  <summary>Details</summary>
Motivation: To enhance software vulnerability detection using Large Language Models by improving input representations for better performance.

Method: The study employs a preprocessing stage to convert Java source code into ASTs, which are then transformed into BT representations, enabling comparisons of vulnerability detection across raw code, AST, and BT formats over 460 samples.

Result: BT representations yield improved recall in short samples and overall better performance on longer samples compared to raw code and AST, which often exceed context size limits in LLMs.

Conclusion: BTs present a compact and effective intermediate representation for vulnerability detection in Java code, beneficial for deployment with quantized LLMs.
...
TL;DR: This paper discusses the issue of endogenous authorization laundering in long-running LLM agents, where persistent memory inaccurately reflects the authorization state, leading to unauthorized actions. It presents EAL-Bench to measure the accuracy of memory in preserving authorization state and evaluates various LLMs across different domains, revealing significant errors in authorization management and suggesting safeguards with inherent trade-offs.


<details>
  <summary>Details</summary>
Motivation: The study aims to address the risks associated with misrepresentation of evolving authorization states in LLM agents due to persistent memory issues, which can lead to unauthorized actions without external attacks.

Method: The authors introduce EAL-Bench to measure how well persistent memory maintains accurate authorization states, examines five LLMs as memory writers, and two as executors across domains like procurement, cybersecurity, and finance.

Result: Under incremental memory updates, it was found that memory writers created false authority in up to 50.2% of unauthorized requests, leading executors to act on false authority in 98.6% of trials.

Conclusion: The findings highlight that persistent memory is critical not just for performance but as a component of an LLM agent's authorization policy, revealing a trade-off between safety and utility when implementing safeguards.
...
TL;DR: This paper examines the security and reliability of EEG signals as biometric credentials against black-box adversarial attacks, developing adaptive attack algorithms to evaluate vulnerabilities across different neural signature methods and recording conditions.


<details>
  <summary>Details</summary>
Motivation: To address the lack of research on the security of EEG-based biometrics and assess vulnerabilities beyond deep-learning classifiers.

Method: Developed adaptive attack algorithms that manipulate stolen EEG recordings without knowledge of the authentication system, tested on six public datasets under various recording conditions.

Result: The results indicate significant variability in vulnerability across different neural signature approaches and recording conditions, with spoofing susceptibility affected by the task performed during EEG recording.

Conclusion: Recommendations are provided to enhance the security of neural signature biometrics, informed by findings from the adversarial tests.
...
TL;DR: The paper introduces the Agent Flight Recorder, a system that tracks agent actions and provides tamper-evident records for accountability in multi-agent environments, ensuring independent verification across organizations.


<details>
  <summary>Details</summary>
Motivation: To address issues of accountability and verification in multi-agent systems where sequential failures occur and traditional methods of dispute resolution are inadequate.

Method: The system captures agent actions as structured events across eight semantic fields, utilizing hash chaining and Merkle batching for tamper evidence and independent verification, with on-chain anchoring for cross-organizational disputes.

Result: The evaluation demonstrated that the system adds minimal latency and overhead while effectively detecting tampering and enabling precise forensic queries.

Conclusion: The Agent Flight Recorder provides a robust integrity framework for agent actions, achieving high precision in verifying actions with negligible performance impact.
...
TL;DR: The paper presents Recourse, a smart-contract protocol that addresses the residual harm from permitted actions of autonomous agents by ensuring compensation and recovery mechanisms.


<details>
  <summary>Details</summary>
Motivation: To mitigate the residual harm caused by failed actions of autonomous agents after they have been authorized and executed.

Method: The protocol binds admitted actions to well-defined scopes, evidence, payouts, and collateral, using typed receipts for computing objective claims and routing subjective claims to arbitration.

Result: The implementation and evaluation of Recourse show that it effectively reduces uncompensated harm compared to traditional methods, demonstrating robust coverage and trust mechanisms.

Conclusion: Recourse enables enhanced compensation for agent side effects while establishing a secure and transparent process for resolving claims, thereby strengthening organizational trust.
...
TL;DR: This paper investigates the privacy aspects of different sampling mechanisms in optimization, focusing on structured participation and comparing them to traditional Poisson subsampling methods.


<details>
  <summary>Details</summary>
Motivation: The increasing adoption of structured participation in practical systems raises questions about their privacy guarantees compared to established methods, making a deeper analysis necessary.

Method: The analysis isolates the probabilistic mechanisms involved, focusing on the conditions under which various schemes, including Poisson subsampling and its alternatives, maintain or lose privacy guarantees, particularly using Rényi divergence.

Result: The findings show that for certain negatively associated participation indicators, structured participation can retain privacy advantages at higher Rényi orders, while also establishing critical thresholds and conditions where Poisson methods may fail.

Conclusion: The results provide a framework for assessing the privacy implications of using Poisson subsampling versus structured participation, emphasizing the need for careful consideration in privacy accounting as these alternative methods gain traction.
...
TL;DR: This paper presents lightweight multi-secret-key protocols for private average aggregation in Federated Learning using RLWE-based Homomorphic Encryption, improving security and efficiency over traditional methods.


<details>
  <summary>Details</summary>
Motivation: To address sensitive information leakage during model update exchanges in Federated Learning and enhance the practicality of Homomorphic Encryption implementations.

Method: The paper introduces multi-secret-key protocols where each client encrypts its update independently, enabling compatibility for homomorphic aggregation without collective public key generation, while managing ciphertext noise effectively.

Result: The proposed protocols significantly decrease ciphertext size and implementation complexity compared to existing MHE-based methods, with efficient communication and runtime performance.

Conclusion: The lightweight approach maintains the integrity of private average aggregation in Federated Learning while reducing overheads associated with large smudging noise, supporting enhanced security and efficiency.
...
TL;DR: C$^2$T-OpenMax enhances device authentication by improving open-set recognition in radio frequency fingerprinting through center-constrained learning and confidence-guided tail modeling.


<details>
  <summary>Details</summary>
Motivation: To enable effective device authentication in varying environments using radio frequency fingerprinting, addressing the challenges posed by data augmentation in open-set recognition.

Method: C$^2$T-OpenMax combines center-constrained learning, which improves intra-class compactness, with confidence-guided tail modeling to refine class representation and OpenMax construction.

Result: C$^2$T-OpenMax outperforms all baselines in accuracy and AUROC, achieving the highest open-set accuracy across eight location groups and significantly improving measures under the largest-openness setting.

Conclusion: The proposed framework significantly enhances open-set recognition performance while retaining the advantages of data augmentation.
...
TL;DR: This paper introduces Implicit Skill-Selection Manipulation (ISM), a technique that exploits semantic relationships to manipulate skill selection in LLM-agent workflows, achieving significantly higher success rates than traditional explicit attacks.


<details>
  <summary>Details</summary>
Motivation: Existing attacks on skill selection strategies in LLM workflows are limited by recognizable manipulation signals; hence, a new method that operates implicitly is needed.

Method: ISM manipulates skill selection by jointly shaping the metadata and prompts, focusing on broadening semantic coverage, enhancing target distinctiveness, and maintaining natural prompt language across multiple task domains and models.

Result: ISM increased the target-selection rate from 15.2% to 63.5%, achieving up to 73.5% in matched comparisons and evading detection from human reviewers and LLM inspectors much more effectively than explicit steering techniques.

Conclusion: ISM presents a viable and stealthy alternative to existing manipulation methods, demonstrating higher effectiveness and lower detectability.
...
TL;DR: This paper proposes a novel approach to enhance Trusted Execution Environments (TEEs) security and efficiency by introducing SIR and SIREN calculi, alongside a compilation method that automates enclave programming.


<details>
  <summary>Details</summary>
Motivation: TEEs provide isolation but lack information-flow security and require labor-intensive manual partitioning, which leads to inefficiencies and errors in application security management.

Method: The authors formalize the SIR calculus with a permissive type system and extend it to SIREN for better security, developing a type-driven compilation process that automates enclave-awareness.

Result: Implementation of SPLITR on various benchmarks shows significant improvements in managing enclave transitions and overall efficiency, with optimizations yielding substantial reductions in transition counts for applications like OpenSSL.

Conclusion: The proposed methodologies enhance the security and performance of applications in TEEs by automating enclave-related programming tasks, thereby minimizing manual effort and improving control over execution boundaries.
...
TL;DR: This paper discusses enhancing the reliability of autobiographical state construction in persistent AI agents by implementing provenance and assertion guardrails to mitigate risks from untrusted inputs and ensure integrity of stored information.


<details>
  <summary>Details</summary>
Motivation: To address the potential risks associated with untrusted inputs in persistent AI agents that can lead to erroneous autobiographical assertions or commitments.

Method: The authors propose a system of typed provenance and assertion guardrails to evaluate and verify the authenticity and reliability of statements made by AI agents, featuring a typed provenance graph and a generate-verify-revise mediator for controlled release of information.

Result: In a testing suite of 24 conformance cases, the proposed typed mediation successfully identified and restrained all identified unsafe opportunities while maintaining supported controls, although it did not pass all evaluations for safety.

Conclusion: The findings confirm the effectiveness of the encoded resolver and mediator obligations in managing potential risks but indicate that further assessments are needed for comprehensive evaluation of language models and retrieval systems.
...
TL;DR: WeaveMark is a novel LLM watermarking scheme that enhances payload capacity, extraction accuracy, and text quality through innovative methods, demonstrating significant improvements over existing techniques.


<details>
  <summary>Details</summary>
Motivation: The need for effective content source tracing in large language models necessitates an improved watermarking approach that manages the trade-off between extraction accuracy, text quality, and payload capacity.

Method: WeaveMark employs multi-bit-per-token spreading, soft-decision error-correcting codes, and unbiased multilayer reweighting, along with additional zero-bit layers for reliable watermark presence detection.

Result: Experiments demonstrate WeaveMark's superiority, achieving an 89.8% match rate for 32-bit messages with 200 tokens, and maintaining 86.0% extraction accuracy against substitution attacks.

Conclusion: WeaveMark significantly enhances watermarking for LLMs, outperforming previous methods while preserving generated text quality.
...
TL;DR: The paper introduces the Agentic Settlement Protocol (ASP) to enhance transaction models for autonomous agents, addressing shortcomings in current HTTP-native protocols, particularly for complex purchases. ASP defines a hold model, verification processes, partial refunds, and revenue share mechanisms to mitigate risks in transactions before fulfillment.


<details>
  <summary>Details</summary>
Motivation: Current payment systems for autonomous agents fail to accommodate complex purchases effectively, as they finalize transactions before service delivery, risking seller finances. ASP aims to rectify this by introducing an escrow system that respects the fulfillment timeline.

Method: The ASP is crafted as an application profile over on-chain authorize-and-capture escrow combined with a fulfillment engine that verifies orders off-chain. It incorporates a three-deadline model and detailed frameworks for verification and refunding.

Result: ASP introduces multiple mechanisms including a hold model with deadlines, a trust hierarchy for capture approvals, partial refund processes, and controls on credit risk, all designed to improve transaction reliability and trust.

Conclusion: The ASP seeks to enhance the commerce experience for autonomous agents and establishes a foundation for further study and implementation, specifically within the XDC Network.
...
TL;DR: This paper presents a framework for tracing synthetic media provenance in generative models on MLaaS platforms without altering the generator.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of tracing synthetic media provenance reliably and without modifying generator architectures due to the rise of generative models on MLaaS.

Method: The paper proposes a self-referential retrosynthesis framework using a jointly optimized encoder-decoder pair for round-trip consistency verification, relying on self-embedding mechanisms.

Result: The approach maintains high visual fidelity in generated images while enabling effective forensic verification of provenance through consistency checks between images.

Conclusion: The framework offers a practical and interpretable tool for generative content provenance, thus enhancing explainability in generative AI forensics.
...
TL;DR: A system combining a fine-tuned YOLOv8 detector and a vision-language model (VLM) effectively solves visual CAPTCHAs, allowing the detector to learn and adapt over time through feedback from the VLM, achieving higher accuracy than either component alone.


<details>
  <summary>Details</summary>
Motivation: To investigate the potential of a solver that improves with use, contrasting it with existing VLMs and specialized detectors, which do not adapt or improve over time.

Method: Combines a fine-tuned YOLOv8 detector with an open-weight VLM, operating entirely from screenshots and OS input events, utilizing a confidence-based router for decision-making.

Result: Achieves 85.4% overall and 84.2% macro accuracy across 16 classes, improving on the performance of both individual components; the system learns from each interaction, adapting to new categories without human annotation.

Conclusion: The research demonstrates that adaptive solvers can rapidly recover from adversarial perturbations in a simulated arms race, challenging the assumption that a failing bot remains ineffective and highlighting the need for CAPTCHA systems to account for adaptive capabilities.
...
TL;DR: The paper presents a systematic evaluation of risk-based alerting (RBA) in security operations centers (SOCs), revealing its effectiveness in prioritizing alerts and reducing false alerts.


<details>
  <summary>Details</summary>
Motivation: To address the challenge posed by high volumes of false alerts in SOCs, which complicate cyberattack detection under resource constraints.

Method: RBA is reformulated as a continuous alert prioritization problem. Five risk hypotheses are identified, formalized as parametrizable modules, and evaluated using the CATS experimentation suite across eight alert datasets.

Result: Combinations of risk hypotheses demonstrated a significant improvement in alert prioritization performance, achieving an average AUROC of 0.92, compared to 0.72 for severity-based prioritization.

Conclusion: RBA can effectively reduce false alerts and the burden on analysts, providing a strong baseline for advanced alert triage methods.
...
TL;DR: This paper investigates the generalization capabilities of machine learning classifiers in intrusion detection systems (IDS) against unseen attacks, highlighting structural weaknesses in current evaluation techniques.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of existing machine learning-based intrusion detection systems, particularly their performance against unseen attacks despite high accuracy in controlled settings.

Method: An experimental design is implemented to evaluate the generalization performance of selected classifiers under two different methodologies and settings, focusing on the impact of dimensionality on attack detection.

Result: The experiments demonstrate the limited ability of models to effectively identify unseen attacks and reveal vulnerabilities in the evaluation methods used in ML-based IDS research.

Conclusion: The paper concludes with the proposal of seven evaluation criteria aimed at improving the assessment of intrusion detection systems, particularly in their response to unknown threats.
...
TL;DR: This paper presents SpiderSapien, an innovative client-centric crawler and security scanner designed to enhance web application security testing by addressing the challenges posed by the interactivity and dynamism of modern applications.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve web application security testing, particularly for modern web applications that are highly interactive and dynamic, which traditional scanners do not adequately address.

Method: The proposed method, SpiderSapien, utilizes high-level, user-facing feedback channels to enable immersive interaction during the crawling process. It employs novel techniques for detecting interactable elements and sequencing UI interactions, along with a large language model for form completion.

Result: SpiderSapien demonstrated a significant improvement in code coverage and vulnerability detection, achieving at least 46% greater coverage than any other scanner and 16% more than the combined results of all scanners. Additionally, it identified XSS vulnerabilities in 7 applications, surpassing the capability of other scanners.

Conclusion: The findings indicate that SpiderSapien effectively enhances the exploration of deep states in modern web applications, providing a robust foundation for future web security scanners.
...
TL;DR: PROSE introduces a method for establishing ownership of large language models through behavioral signatures based on semantic organization rather than fixed query-key associations.


<details>
  <summary>Details</summary>
Motivation: The increasing redistribution and adaptation of large language models challenge traditional methods of establishing model ownership, necessitating a more robust identification method that operates through black-box interactions.

Method: PROSE replaces static fingerprints with a dynamic, semantic-based approach, employing domain-specific semantic templates and mixed fine-tuning to enhance detection of model ownership through behavioral patterns in responses.

Result: PROSE demonstrates a 100% detection rate for model ownership without false positives across various model architectures and remains effective despite modifications.

Conclusion: The proposed method preserves model utility while offering robust ownership verification through higher semantic-level behavioral signatures.
...
TL;DR: The paper addresses the risks posed by reusable agent skills in LLMs, formalizes Skill Policy Integrity, and introduces SkillShift, a framework for covertly steering agent policies while maintaining effectiveness and inconspicuousness.


<details>
  <summary>Details</summary>
Motivation: To mitigate supply-chain risks associated with third-party reusable skills in LLMs, ensuring that agent behavior remains aligned with user-authorized objectives.

Method: The paper presents SkillShift, a constrained black-box framework that uses plausible policy edits and hierarchical validation to steer agent policies covertly.

Result: SkillShift enables covert policy steering with high attacker-favored selection rates (81.33% and 63.33%) while maintaining 100% utility-preserving rates and successful transfer across varied LLM environments.

Conclusion: The study underscores the need for behavioral auditing of reusable skills due to their potential to be stealthily manipulated, as existing scanners fail to detect such covert skills.
...
TL;DR: This paper evaluates the security of a selective coordinate encryption framework for point clouds against machine learning reconstruction attacks and examines the impact of encryption granularity.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess if the unencrypted data in selectively encrypted point clouds allows for effective reconstruction, posing security risks in immersive video applications.

Method: The authors test reconstruction attacks using machine learning models, specifically PointNet and Random Forest, under two encryption schemes: encrypting all coordinates and encrypting every second coordinate.

Result: Results indicate that complete encryption of coordinates maintains security, but with every second coordinate encrypted, reconstruction attacks become feasible due to leaked information from neighbor relationships.

Conclusion: The study concludes that the effectiveness of selective coordinate encryption is heavily influenced by the granularity of the encryption applied.
...
TL;DR: This paper addresses the gap between abstract exploit strategies and the technical operations required for kernel exploitation by proposing a systematic characterization of exploit primitives and introducing 	extsc{PrimSynth}, a framework for automated discovery and synthesis of exploit primitives for Linux kernel vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between high-level exploit strategies and the detailed technical operations necessary for automated kernel exploitation, which remains unresolved despite previous research efforts.

Method: The paper formalizes six classes of exploit primitives and proposes a new representation for exploit strategies that combines upgrading strategies with code synthesis rules. 	extsc{PrimSynth}, a framework, uses coordinated agents in an iterative loop for discovering and validating exploit primitives based on memory corruption vulnerabilities.

Result: 	extsc{PrimSynth} was evaluated on 16 real-world Linux kernel CVEs, achieving a 100% match rate in primitive extraction and successfully synthesizing exploitation chains with an 82.4% SSR when guided by public PoCs and a 61.3% SSR without such guidance.

Conclusion: The proposed framework enables effective extraction and synthesis of exploit primitives, demonstrating its potential in automating kernel vulnerability exploitation.
...
TL;DR: The paper introduces ACLE-MCP, an architecture that addresses the post-authorization execution trust gap in remote model context protocols, ensuring secure invocation of external tools by binding authorization to current workload states.


<details>
  <summary>Details</summary>
Motivation: To overcome the vulnerabilities in OAuth authorization that allow unauthorized tool calls due to shifting workloads or outdated appraisals, and to ensure that authorized calls remain trust-bound to expected execution environments.

Method: The paper presents ACLE-MCP, which implements a short-lived, sender-constrained capability lease system that ties together authorization, workload appraisal, and execution admission, using an Execution Gate for secure invocation.

Result: Controlled experiments demonstrate that ACLE-MCP successfully blocks all evaluated post-authorization attack vectors while maintaining functionality, contrary to weaker authorization methods which leave vulnerabilities.

Conclusion: ICLE-MCP proves that binding call authority to the current workload state enhances security for remote tool use beyond what OAuth alone can provide, with a manageable increase in processing latency.
...
TL;DR: This paper introduces protocols for card-based cryptography focusing on player simulation in games with a publicly known ranking of cards, specifically featuring the Play-Minimum and Sorting protocols.


<details>
  <summary>Details</summary>
Motivation: The study aims to advance understanding in card-based cryptography by addressing unique constraints of player simulation in card games.

Method: The paper proposes two protocols: Play-Minimum for selecting minimum or maximum cards securely, and Sorting for arranging a player's hand in nondecreasing order.

Result: The protocols enable secure selection and ordering of cards while respecting the constraints of player simulation, applicable to games with distinct and non-distinct card values.

Conclusion: These protocols serve as foundational elements for generic computational primitives in virtual player simulations, expanding the potential of card-based cryptography.
...
TL;DR: SPADE is a novel dataset created to enhance intrusion detection systems for connected vehicles, specifically focusing on the integrity of Signal Phase and Timing messages, amidst potential application-layer attacks.


<details>
  <summary>Details</summary>
Motivation: To address the gap in the integrity evaluation of SPaT messages from the onboard connected vehicle perspective, as existing research primarily targets infrastructure or V2V messaging misbehavior.

Method: SPADE is generated using Eclipse MOSAIC, employing runtime attack injections categorized into six attack classes and one benign class, across various intersection geometries and conditions to create a comprehensive dataset.

Result: The dataset consists of approximately 1,890,000 labeled timestep records, integrating SPaT message fields, onboard camera data, and V2V peer information across 40 features to facilitate the detection of attacks versus environmental issues.

Conclusion: SPADE aims to support reproducible IDS research in C-V2X security by providing a publicly accessible dataset and related resources.
...
TL;DR: CodePoisonRAG is a framework for targeted knowledge poisoning in Retrieval-Augmented Code Generation (RACG), enabling black-box attackers to inject vulnerabilities without direct access to the victim's system, demonstrating significant attack success rates.


<details>
  <summary>Details</summary>
Motivation: The need to understand and mitigate vulnerabilities introduced in RACG due to the incorporation of external artifacts which can be maliciously crafted.

Method: CodePoisonRAG utilizes CWE-specific Vulnerability Injection to craft poisoned artifacts that integrate vulnerabilities while maintaining task alignment and employs Semantic Mislabeling to misrepresent safety without correcting the unsafe behavior.

Result: The study generates 85 poisoned artifacts which demonstrate high attack success rates across multiple code generators, highlighting the effectiveness of the method in influencing outputs despite limited access to victim systems.

Conclusion: RACG is susceptible to specifically crafted vulnerabilities, allowing attackers to intentionally propagate weaknesses through targeted knowledge poisoning strategies.
...
TL;DR: The paper introduces EFFECTBOUND, a method for assessing policy-relative effect closure to prevent unauthorized operations during contract execution in software systems.


<details>
  <summary>Details</summary>
Motivation: The need to ensure authorized actions do not lead to unintended effects that an application would reject, maintaining contract adherence while completing necessary operations.

Method: EFFECTBOUND employs an evidence-supported finite contract to evaluate if an interface can accurately report closure during operational completion, reducing the problem to finite control with hidden state.

Result: In practical tests on platforms like GitHub, Kubernetes, NATS, and Kafka, various failure modes of closure were identified, where interfaces inaccurately reported state or allowed unauthorized effects due to timing or control issues.

Conclusion: By introducing a gate mechanism, the method effectively prevents new uses of revoked authority, thus ensuring that effects of earlier authorizations do not reach states that the application would reject.
...
TL;DR: This paper presents a randomness-efficient method for answering linear queries under differential privacy with fewer random bits, improving error bounds compared to previous work.


<details>
  <summary>Details</summary>
Motivation: To enhance the efficiency of answering linear queries with differential privacy while minimizing the amount of randomness required.

Method: The authors introduce a modified $
orm K$-norm mechanism that utilizes $O(	ext{log } d)$ random bits to achieve $O(d / 	ext{ε})$ error for $d$ linear queries.

Result: The method achieves improved error performance over existing algorithms in the context of $	ext{ℓ}_	ext{∞}$-error, becoming optimal for $	ext{ε} 	ext{ ≤ } 1/d$.

Conclusion: They also present a computationally efficient version of the algorithm, though at the cost of increased error.
